...
def benchmark():
    ...
    with open(f"benchmark-{time.strftime('%Y%m%d%H%M%S')}.csv", "wb", buffering=0) as file:
        ...
        for content_type in ContentType:
            ...
            for number_of_peers in range(2, len(network.nodes) + 1):
                publisher, *subscribers = choose_peers(network, number_of_peers)
                tracks = generate_content(
                    track_id, publisher, subscribers, content_type)
                ...
                for single_track_optimizer_type in SingleTrackOptimizerType:
                    ...
                    print(f"Running benchmark for {content_type.name} with {number_of_peers} peers using {single_track_optimizer_type.name}")
...
async def collect_optimization_info(network, tracks, multi_track_optimizer):
    start = time.time()
